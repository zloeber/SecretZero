...
- **`secretzero web` tabbed views:** dashboard now supports tabs for **Dashboard** (operations), **Secretfile.yml** (full manifest text), and **Graph** (Mermaid rendering plus generated JSON graph view with flow/detailed/architecture Mermaid variants).
...
- **Agent skill guidance refreshed:** `skills/secretzero/SKILL.md` now includes explicit Secretfile authoring rules that prioritize whole-secret modeling and straightforward template usage, plus discovery-assisted least-privilege `provider_identity` policy authoring (account/region-focused guardrails).
