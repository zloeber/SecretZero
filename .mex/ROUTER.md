...
- `secretzero backup`: `backup create` defaults to plain local JSON backup payloads (stdout unless `--output` is given), blocks plain mode under `SZ_AGENT`, and uses `--encrypted` to opt into the SOPS+AGE file workflow. When no environment is selected, backup/restore now fan out across every `environments.profiles` lane and still allow single-environment targeting with `--environment`.
...
- **Supply-chain hardening defaults:** CI/workflows now install `uv` via SHA-pinned `astral-sh/setup-uv`, use frozen dependency resolution (`uv sync --frozen ...`) in release/test/docs/agent pre-commit paths, pin PyPI publish action to a specific commit SHA, and avoid mutable `wrangler@latest` installs in docs publishing.
