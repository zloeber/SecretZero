...
- Dashboard supports **tabbed views** via query params: `tab=dashboard|secretfile|interpolated|graph`, plus graph tab toggles `graph_view=mermaid|json` and Mermaid `graph_type=flow|detailed|architecture`. Keep dashboard actions on the Dashboard tab; Secretfile / interpolated / Graph tabs are read-only. The **Manifest (interpolated)** tab dumps the in-memory validated `Secretfile` (aligned with `secretzero render`) and can expose literal static values—same caution as sharing manifest text on a network-bound server.
- **Graph tab:** Mermaid `linkStyle` indices match edge declaration order (all generator→secret edges first, then secret→target edges). Edge colors use lockfile state (`synced` / `pending` / `drift`) plus optional **unknown** (yellow) when `preflight_provider_identity_policies` fails for that target’s provider (same preflight passed into `generate_graph(..., identity_preflight=...)`). Subgraph titles show `provider/kind` only; inner nodes carry destination/detail labels.
- **Dashboard flow arrows / Force to target:** When identity policies fail for a lane’s provider, the lane arrow uses the **unknown** color, **Force to target** is `disabled` with a tooltip summarizing policy rows; lockfile `sync_state` on the lane is unchanged for filtering.
