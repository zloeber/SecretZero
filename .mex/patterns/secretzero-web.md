...
- Dashboard supports **tabbed views** via query params: `tab=dashboard|secretfile|graph`, plus graph tab toggles `graph_view=mermaid|json` and Mermaid `graph_type=flow|detailed|architecture`. Keep dashboard actions on the Dashboard tab; Secretfile/Graph tabs are read-only.
