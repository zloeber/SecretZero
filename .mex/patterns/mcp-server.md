...
## Steps
1. Add or change tools in `create_mcp_server()` using official `mcp.server.MCPServer` (SDK 2.x; formerly `FastMCP`).
...
7. Optional extra: `secretzero[mcp]` (`mcp>=2.0.0`). Prefer latest SDK majors once tool registration/APIs are migrated.
...
- SDK 2.x `call_tool` returns `CallToolResult` (`structured_content` / `content`); use `structured_output=True` on tools that return dict metadata.
