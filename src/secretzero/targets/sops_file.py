"""Target adapter for storing secrets via the SOPS provider."""
...
class SopsFileTarget(BaseTarget):
    """Store/retrieve secrets in a SOPS-encrypted file."""

    def __init__(self, provider: Any, config: dict[str, Any] | None = None) -> None:
        super().__init__(provider, config or {})
        if not isinstance(self.provider, SopsProvider):
            raise ValueError("sops_file target requires a sops provider")
