"""Target adapter for storing secrets via the git-crypt provider."""
...
class GitCryptFileTarget(BaseTarget):
    """Store/retrieve secrets in git-crypt-managed files."""

    def __init__(self, provider: Any, config: dict[str, Any] | None = None) -> None:
        super().__init__(provider, config or {})
        if not isinstance(self.provider, GitCryptProvider):
            raise ValueError("git_crypt_file target requires a git_crypt provider")
